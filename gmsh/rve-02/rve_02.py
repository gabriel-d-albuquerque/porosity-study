...
sys.path.insert(1, r'C:\Users\gabri\abaqus_plugins\PBC_novtx_2D')

import periodic_bcs_novtx_2D

for i in range(8):
    inpf_name = '../models/rve-02-P' + str(i) + '-2D.inp'
    m_name = 'rve-02-P' + str(i) + '-2D'
    j_name = m_name
    ...
    periodic_bcs_novtx_2D.pbc_tool(model_name = m_name, opt = '4. Sets + RFs + coefs + eq')
    ...
    path = './'
    getState(path + j_name + '.odb', out_name, P[i])
    
mdb.saveAs('./rve-02.cae')
